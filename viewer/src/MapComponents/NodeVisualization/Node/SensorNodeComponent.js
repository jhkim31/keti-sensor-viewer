...
const SensorNodeComponent = ({ 
    ...
    const map_img_dom = document.getElementById(properties.main_state.selected_factory + "_map_" + properties.main_state.map_index)
    const [isShown, set_isShown] = useState(false);    
    const width = window.innerWidth - 200;
    const height = config.layout.topology_component.height;

    const left = properties.main_data.sensor_position[item].x * width;
    const top = properties.main_data.sensor_position[item].y * height; 
    
    let service_time = properties.main_data.selected_factory_sensor_data[item].service.timestamp;
